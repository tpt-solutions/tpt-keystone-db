...
fn milestone_replay_failed_session_traces_root_cause_and_audit_report_is_tamper_evident() {
    ...
    tracer.record_decision(agent, session, "decided to gather sources, then email a summary").unwrap();
    audit.record(session, agent, "policy_check", "search query passed content policy").unwrap();
    tracer.record_tool_call(agent, session, "web_search", "\"quarterly earnings\"", "[3 results]").unwrap();
    metrics.record(agent, session, 120.0, 400, true).unwrap();
    audit.record(session, agent, "policy_check", "recipient list passed PII policy").unwrap();
    let fail_offset = tracer.record_error(agent, session, Some("send_email"), "SMTP connection refused").unwrap();
    metrics.record(agent, session, 50.0, 0, false).unwrap();
    audit.record(session, "system", "session_end", "session ended in failure").unwrap();
    ...
    let root_cause = engine.find_first_error(session).unwrap().expect("session has a recorded failure");
    ...
    let result = execute_query(&format!("SELECT kind, tool_name FROM mirror_session_events('{session}') ORDER BY evt_offset"), db.clone()).unwrap();
    ...
    assert!(report.tamper_evident, "an untouched chain auto-generates as tamper-evident");
    ...
    let rate = metrics.success_rate(agent, 0, crate::synapse::now_ms() + 60_000).unwrap().unwrap();
    assert!((rate - 0.5).abs() < 1e-9);

    let sql_metrics = execute_query(
        &format!("SELECT success FROM mirror_agent_metrics('{agent}', 0, {}) ORDER BY ts", crate::synapse::now_ms() + 60_000),
        db.clone(),
    ).unwrap();
    assert_eq!(sql_metrics.rows.len(), 2);
    assert_eq!(cell_text(&sql_metrics.rows[1][0]), "0", "the failed call's metric row");
}
