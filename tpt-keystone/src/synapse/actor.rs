...
use crate::synapse::{cell_i64, cell_text, col, decode_cell, encode_cells, int_cell, now_ms, text_cell};
...
enum AgentMessage {
    Deliver { payload: String, reply: oneshot::Sender<String> },
    Pause,
    Resume,
    ...
    while let Some(msg) = rx.recv().await {
        match msg {
            ...
            AgentMessage::Checkpoint(reply) => {
                ...
                last_checkpoint = state.short_term.get("__checkpoint").cloned().unwrap_or(last_checkpoint);
...
impl AgentHandle {
    ...
    pub async fn send(&self, payload: &str) -> Result<String> {
        ...
        self.tx.send(AgentMessage::Deliver { payload: payload.to_string(), reply: reply_tx }).await
            .map_err(|_| anyhow!("agent \"{}\" mailbox closed", self.id))?;
        reply_rx.await.map_err(|_| anyhow!("agent \"{}\" dropped without replying", self.id))
    }

    pub async fn pause(&self) -> Result<()> {
        self.tx.send(AgentMessage::Pause).await.map_err(|_| anyhow!("agent \"{}\" mailbox closed", self.id))
    }

    pub async fn resume(&self) -> Result<()> {
        self.tx.send(AgentMessage::Resume).await.map_err(|_| anyhow!("agent \"{}\" mailbox closed", self.id))
    }

    pub async fn checkpoint(&self) -> Result<String> {
        let (reply_tx, reply_rx) = oneshot::channel();
        self.tx.send(AgentMessage::Checkpoint(reply_tx)).await
            .map_err(|_| anyhow!("agent \"{}\" mailbox closed", self.id))?;
        reply_rx.await.map_err(|_| anyhow!("agent \"{}\" dropped without replying", self.id))
    }

    pub async fn terminate(&self) -> Result<()> {
        self.tx.send(AgentMessage::Terminate).await.map_err(|_| anyhow!("agent \"{}\" mailbox closed", self.id))
    }
}
...
impl AgentRegistry {
    pub fn new(db: Arc<Database>) -> Result<Self> {
        ...
        Ok(Self { db, handles: Mutex::new(HashMap::new()) })
    }
    ...
    fn persist(&self, id: &str, name: &str, status: AgentStatus, checkpoint: &str, created_at: i64) -> Result<()> {
        let cells = vec![
            text_cell(id),
            text_cell(name),
        ...
        self.db.write(AGENTS_TABLE, id.as_bytes(), &encode_cells(&cells))
    }
    ...
    pub fn spawn(&self, id: impl Into<String>, name: &str, step: StepFn) -> Result<AgentHandle> {
        ...
        tokio::spawn(actor_loop(AgentState { short_term: HashMap::new() }, step, rx));
    ...
    pub fn resume_from_checkpoint(&self, id: &str, name: &str, step: StepFn) -> Result<AgentHandle> {
        let checkpoint = self.db.read(AGENTS_TABLE, id.as_bytes())?
        ...
        tokio::spawn(actor_loop(AgentState { short_term: initial }, step, rx));
        let handle = AgentHandle { id: id.to_string(), tx };
    ...
    pub async fn set_status(&self, id: &str, status: AgentStatus) -> Result<()> {
        ...
        let row = self.db.read(AGENTS_TABLE, id.as_bytes())?
...
mod tests {
    ...
    async fn spawn_send_and_receive_reply() {
        ...
        let handle = registry.spawn("a1", "echo-agent", Box::new(|_state, msg| format!("echo:{msg}"))).unwrap();
    ...
    async fn short_term_state_persists_across_messages_within_a_session() {
        let (db, _b, _l) = test_db();
        let registry = AgentRegistry::new(db).unwrap();
        let handle = registry.spawn("a1", "counter", Box::new(|state, _msg| {
            let n: u64 = state.short_term.get("count").and_then(|s| s.parse().ok()).unwrap_or(0) + 1;
    ...
    async fn pause_drops_messages_and_resume_processes_again() {
        let (db, _b, _l) = test_db();
        let registry = AgentRegistry::new(db).unwrap();
        let handle = registry.spawn("a1", "echo", Box::new(|_s, msg| msg.to_string())).unwrap();
    ...
    async fn checkpoint_persists_and_resume_from_checkpoint_reloads_it() {
        let (db, _b, _l) = test_db();
        let registry = AgentRegistry::new(db).unwrap();
        let handle = registry.spawn("a1", "saver", Box::new(|state, msg| {
            state.short_term.insert("__checkpoint".to_string(), format!("saw:{msg}"));
            "ok".to_string()
        })).unwrap();
        ...
        let handle2 = registry.resume_from_checkpoint("a1", "saver", Box::new(|state, _msg| {
            state.short_term.get("__checkpoint").cloned().unwrap_or_default()
        })).unwrap();
    ...
    async fn terminate_removes_from_live_registry() {
        let (db, _b, _l) = test_db();
        let registry = AgentRegistry::new(db).unwrap();
        registry.spawn("a1", "x", Box::new(|_s, m| m.to_string())).unwrap();
        assert!(registry.get("a1").is_some());
        registry.set_status("a1", AgentStatus::Terminated).await.unwrap();
        assert!(registry.get("a1").is_none());
        let all = registry.list_all().unwrap();
        assert_eq!(all.iter().find(|(id, ..)| id == "a1").unwrap().2, "terminated");
    }
}
