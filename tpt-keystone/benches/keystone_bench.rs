...
fn test_db() -> (Arc<Database>, tempfile::TempDir, tempfile::TempDir) {
    ...
    let lease = Arc::new(LeaseManager::new(store.clone(), "db", "bench-node".into(), Duration::from_secs(30)));
    ...
    group.bench_function("insert_500_rows", |b| {
        b.iter_batched(
            || {
                ...
                q(&db, "CREATE TABLE bench_ins (id INT4, name TEXT, score FLOAT8)");
                (db, bucket, local)
            },
            |(db, _bucket, _local)| {
                for i in 0..N {
                    q(&db, &format!("INSERT INTO bench_ins VALUES ({i}, 'row-{i}', {})", i as f64 * 1.5));
                }
            },
...
fn bench_point_select(c: &mut Criterion) {
    ...
    q(&db, "CREATE TABLE bench_pt (id INT4, name TEXT, score FLOAT8)");
    for i in 0..5000 {
        q(&db, &format!("INSERT INTO bench_pt VALUES ({i}, 'row-{i}', {})", i as f64 * 1.5));
    }
    ...
    group.bench_function("select_by_id_5000_row_table", |b| {
        ...
        b.iter(|| {
            ...
            execute_query(&format!("SELECT * FROM bench_pt WHERE id = {i}"), db.clone()).unwrap()
        });
    });
    group.finish();
}
...
fn bench_full_scan(c: &mut Criterion) {
    let (db, _bucket, _local) = test_db();
    q(&db, "CREATE TABLE bench_scan (id INT4, name TEXT, score FLOAT8)");
    for i in 0..5000 {
        q(&db, &format!("INSERT INTO bench_scan VALUES ({i}, 'row-{i}', {})", i as f64 * 1.5));
    }
...
fn bench_vector_knn(c: &mut Criterion) {
    ...
    for i in 0..2000 {
        ...
        q(&db, &format!("INSERT INTO bench_vec VALUES ({i}, '[{x},{y},{z}]')"));
    }
    q(&db, "CREATE INDEX ON bench_vec USING VECTOR (embedding) WITH (metric = 'l2')");

    let mut group = c.benchmark_group("vector_knn");
    group.bench_function("knn_10_of_2000", |b| {
        b.iter(|| execute_query("SELECT * FROM vector_search('bench_vec', 'embedding', '[0.5,0.5,0.5]', 10)", db.clone()).unwrap());
    });
    group.finish();
}
...
fn bench_bm25_search(c: &mut Criterion) {
    ...
    let words = ["rust", "systems", "programming", "database", "engine", "storage", "query", "index"];
    for i in 0..2000 {
        let body = (0..12).map(|j| words[(i * 7 + j) % words.len()]).collect::<Vec<_>>().join(" ");
        q(&db, &format!("INSERT INTO bench_fts VALUES ({i}, '{body}')"));
    }
    ...
    group.bench_function("bm25_top10_of_2000", |b| {
        b.iter(|| db.fts_search_bm25("bench_fts", "body", "rust database engine", 10).unwrap());
    });
    group.finish();
}
