...
surface for C/C++ interop, and optional access to `tpt-canvas`'s reactive core.
...
| `canvas` | no | Re-exports `tpt-canvas`'s reactive primitives (`Signal`, `create_effect`, `create_memo`) |
...
- No native GPU / WebGPU rendering — the `canvas` feature exposes reactive primitives only; `Canvas.*`
  components only render inside a browser (see `tpt-canvas`).
